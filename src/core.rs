...
fn core_repr(v: ValueList, names: &NamePool) -> ValueResult {
    Ok(Value::Str(format!("{}", Printer::repr_name(v.get(0).unwrap_or(&Value::Nil), names))))
}
...
pub fn ns() -> Vec<(&'static str, Arity, fn(ValueList, &NamePool) -> ValueResult)>{
    vec![
        ...
        ("string->chars", Arity::Exact(1), core_string_to_chars),
        ...
        ("chars/slice", Arity::Range(2, 3), core_chars_slice),
    ]
}
